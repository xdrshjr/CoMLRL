...
class MARLOOTrainer(MAGRPOTrainer):
    ...
    def _compute_loss_with_gradients(self, agent, completions_data, returns):
        """
        Compute loss with a return leave-one-out (RLOO) baseline for advantages.
        ...
        # RLOO advantage: A_g = R_g - mean_{k != g}(R_k)
        # Equivalent vectorized form: A = (G * R - sum(R)) / (G - 1)
        G = returns_tensor.numel()
        if G <= 1:
            # Fallback (shouldn't happen; num_generations >= 2 is enforced)
            advantages = returns_tensor - returns_tensor.mean()
        else:
            sum_ret = returns_tensor.sum()
            advantages = (returns_tensor * G - sum_ret) / (G - 1)
